...
//  Copyright © 2022年 R7. All rights reserved.
//
...
- (void)initDefaultConfign{
    ...
    _norColor = rgba(139, 139, 139, 1);
    _selColor = rgba(252, 36, 74, 1);
...
- (void)setContentView:(NSArray *)contenArr titleArr:(NSArray *)titleArr{
    ...
    self.scroller.frame = CGRectMake(0, 0, ScreenWidth,self.frameRect.size.height + self.frameRect.origin.y + 10);
    self.scroller.layer.cornerRadius = 6;
    self.scroller.layer.masksToBounds = YES;
    self.scroller.backgroundColor = [UIColor blackColor];
}

- (NSInteger)viewHeight{
    return  self.frameRect.size.height + self.frameRect.origin.y + 10;
}
...
- (CGRect)setSignView:(NSArray *)dataAr andTitle:(NSString *)titleStr andFrame:(CGRect)frame andGroupId:(NSInteger)groupId{
    ...
    [dataAr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [but setTitleColor:_norColor forState:UIControlStateNormal];
        [but setTitleColor:_selColor forState:UIControlStateSelected];
